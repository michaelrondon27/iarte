	...
	*/
	function listar(cuadro){
		...
		var table=$("#tabla").DataTable({
			...
			"ajax":{
				"method":"POST",
				"url":carpeta+"admin/estadosciviles/listing",
			...
			"buttons":[
				{
					...
					"titleAttr": "Agregar Estado Civil",
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_agregar(){
		$("#estado_civil_registrar").val("").focus();
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function registrar(){
		$("#form_registrar").submit(function(e){
			...
			$.ajax({
				url:carpeta+'admin/estadosciviles',
				...
				error: function (repuesta) {
			        ...
			        if(typeof errores=='undefined'){
				    	...
				    	for(var i=0; i<errores.estado_civil.length; i++){
				        	html+='<li>'+errores.estado_civil[i]+'</li>';
				        }
				    }
			        html+='</div>';
					$("#alert").html(html);
	...
	*/
	function editar(tbody, table){
		...
		$(tbody).on("click", "span.editar", function(){
			...
			document.getElementById("estado_civil_actualizar").value=data.estado_civil;
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_editar(){
		$("#estado_civil_actualizar").val("").focus();
		$("#id_actualizar").val("");
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function actualizar(){
		$("#form_actualizar").submit(function(e){
			...
			$('input[type="submit"]').attr('disabled','disabled');
			$.ajax({
				url:carpeta+'admin/estadosciviles/'+document.getElementById('id_actualizar').value,
				headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
				type:'PUT',
				dataType:'JSON',
				data:form,
		...
		$(tbody).on("click", "span.eliminar", function(){
			...
			function(isConfirm){
			  	if (isConfirm) {
			    	...
			    	scrollToTop();
			    	$.ajax({
						url:carpeta+'admin/estadosciviles/'+data.id,
						headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
						type: 'DELETE',
						dataType: 'JSON',
						data:{
