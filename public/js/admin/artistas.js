			...
			"buttons":[
				{
        		...
        		},
      			{
		           	...
		           	header: true,
		           	title: 'IARTE - Listado de Artistas',
	...
	function enviarFormulario(form, url, alert){
		$(form).submit(function(e){
			...
			if(form=="#form_asignar"){
				url="admin/artistas/"+document.getElementById('artista_id').value+"/asignar";
			}
	...
	*/
	function lockUnlockArtistas(accion, msj, url, tbody, table){
		$(tbody).on("click", "span."+accion, function(){
			...
			swal({
			  	title: "¿Esta seguro de "+accion+" al artista?",
			  	type: "warning",
		...
		$.ajax({
			...
			success: function(respuesta){
				...
				/* Verificar el tipo de usuario y el tipo de artista para ver si puede editar las imagenes */
					...
					var id_artista=document.getElementById('id').value;
					console.log(id_artista==respuesta.id);
				...
				/* Mostrar Estadisticas */
					...
					conteo("#visitas");
					$("#promedio").html(respuesta.promedioVisitaCatalogo);
					conteo("#promedio");
				/* ---------------------------------- */
				...
				if(document.getElementById('control').value=='edit'){
					/* Mostrar datos personales del artista */
						...
						if(respuesta.tipo==0 && (perfil=="Administrador" || perfil=="Auditor")){
							...
							respuesta.profesiones.forEach(function(profesion, index){
								$("#p-profesiones").append("<span class='badge'>"+profesion.profesion+"</span>");
							});
							respuesta.disciplinas.forEach(function(disciplina, index){
								$("#p-disciplinas").append("<span class='badge'>"+disciplina.disciplina+"</span>");
							});
							...
							if(respuesta.pais_muerte[0].pais!=""){
								$("#p-pais_muerte").html("<b>País de Muerte: "+respuesta.pais_muerte[0].pais+"</b>");
								$("#p-pais_muerte").show();
							}
						}
					/* ---------------------------------- */
