	...
	*/
	function listar(cuadro){
		...
		var table=$("#tabla").DataTable({
			...
			"ajax":{
				"method":"POST",
				"url":carpeta+"admin/tiposmuseos/listing",
			...
			"buttons":[
				{
					...
					"titleAttr": "Agregar Tipo de Museo",
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_agregar(){
		$("#tipo_museo_registrar").val("").focus();
	}
/* ------------------------------------------------------------------------------- */
			...
			$.ajax({
				...
				error: function (repuesta) {
			        ...
			        if(typeof errores=='undefined'){
				    	...
				    	for(var i=0; i<errores.tipo_museo.length; i++){
				        	html+='<li>'+errores.tipo_museo[i]+'</li>';
				        }
				    }
			        html+='</div>';
					$("#alert").html(html);
	...
	*/
	function editar(tbody, table){
		...
		$(tbody).on("click", "span.editar", function(){
			...
			document.getElementById("tipo_museo_actualizar").value=data.tipo_museo;
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_editar(){
		$("#tipo_museo_actualizar").val("").focus();
		$("#id_actualizar").val("");
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function actualizar(){
		$("#form_actualizar").submit(function(e){
			...
			$('input[type="submit"]').attr('disabled','disabled');
			$.ajax({
				url:carpeta+'admin/tiposmuseos/'+document.getElementById('id_actualizar').value,
				headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
				type:'PUT',
				dataType:'JSON',
				data:form,
		...
		$(tbody).on("click", "span.eliminar", function(){
			...
			function(isConfirm){
			  	if (isConfirm) {
			    	...
			    	scrollToTop();
			    	$.ajax({
						url:carpeta+'admin/tiposmuseos/'+data.id,
						headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
						type: 'DELETE',
						dataType: 'JSON',
						data:{
