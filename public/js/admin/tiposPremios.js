	...
	*/
	function listar(cuadro){
		...
		var table=$("#tabla").DataTable({
			...
			"ajax":{
				"method":"POST",
				"url":carpeta+"admin/tipospremios/listing",
			...
			"buttons":[
				{
					...
					"titleAttr": "Agregar Tipo de Premio",
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_agregar(){
		$("#tipo_premio_registrar").val("").focus();
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function registrar(){
		$("#form_registrar").submit(function(e){
			...
			$.ajax({
				url:carpeta+'admin/tipospremios',
				...
				error: function (repuesta) {
			        ...
			        if(typeof errores=='undefined'){
				    	...
				    	for(var i=0; i<errores.tipo_premio.length; i++){
				        	html+='<li>'+errores.tipo_premio[i]+'</li>';
				        }
				    }
			        html+='</div>';
					$("#alert").html(html);
	...
	*/
	function editar(tbody, table){
		...
		$(tbody).on("click", "span.editar", function(){
			...
			document.getElementById("tipo_premio_actualizar").value=data.tipo_premio;
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_editar(){
		$("#tipo_premio_actualizar").val("").focus();
		$("#id_actualizar").val("");
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function actualizar(){
		$("#form_actualizar").submit(function(e){
			...
			$('input[type="submit"]').attr('disabled','disabled');
			$.ajax({
				url:carpeta+'admin/tipospremios/'+document.getElementById('id_actualizar').value,
				headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
				type:'PUT',
				dataType:'JSON',
				data:form,
		...
		$(tbody).on("click", "span.eliminar", function(){
			...
			function(isConfirm){
			  	if (isConfirm) {
			    	...
			    	scrollToTop();
			    	$.ajax({
						url:carpeta+'admin/tipospremios/'+data.id,
						headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
						type: 'DELETE',
						dataType: 'JSON',
						data:{
