	...
	*/
	function listar(cuadro){
		...
		var table=$("#tabla").DataTable({
			...
			"ajax":{
				"method":"POST",
				"url":carpeta+"admin/estados/listing",
			...
			$.ajax({
				...
				error: function (repuesta) {
			        ...
			        if(typeof errores=='undefined'){
				    	...
				    	for(var i=0; i<errores.estado.length; i++){
				        	html+='<li>'+errores.estado[i]+'</li>';
				        }
				    }
			        html+='</div>';
					$("#alert").html(html);
	...
	*/
	function editar(tbody, table){
		...
		$(tbody).on("click", "span.editar", function(){
			...
			document.getElementById("estado_actualizar").value=data.estado;
	...
	*/
	function actualizar(){
		$("#form_actualizar").submit(function(e){
			...
			$('input[type="submit"]').attr('disabled','disabled');
			$.ajax({
				url:carpeta+'admin/estados/'+document.getElementById('id_actualizar').value,
				headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
				type:'PUT',
				dataType:'JSON',
				data:form,
		...
		$(tbody).on("click", "span.eliminar", function(){
			...
			function(isConfirm){
			  	if (isConfirm) {
			    	...
			    	scrollToTop();
			    	$.ajax({
						url:carpeta+'admin/estados/'+data.id,
						headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
						type: 'DELETE',
						dataType: 'JSON',
						data:{
