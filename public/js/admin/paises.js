	...
	*/
	function listar(cuadro){
		...
		var table=$("#tabla").DataTable({
			...
			"ajax":{
				"method":"POST",
				"url":carpeta+"admin/paises/listing",
			...
			"buttons":[
				{
					"text": "<i class='fa fa-flag'></i>",
					"titleAttr": "Agregar País",
			...
			$.ajax({
				...
				error: function (repuesta) {
			        ...
			        if(typeof errores=='undefined'){
				    	...
				    	for(var i=0; i<errores.pais.length; i++){
				        	html+='<li>'+errores.pais[i]+'</li>';
				        }
				    }
			        html+='</div>';
					$("#alert").html(html);
	...
	*/
	function editar(tbody, table){
		...
		$(tbody).on("click", "span.editar", function(){
			...
			document.getElementById("pais_actualizar").value=data.pais;
	...
	*/
	function actualizar(){
		$("#form_actualizar").submit(function(e){
			...
			var form=$(this).serialize();
			$.ajax({
				url:carpeta+'admin/paises/'+document.getElementById('id_actualizar').value,
				headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
				type:'PUT',
				dataType:'JSON',
				data:form,
		...
		$(tbody).on("click", "span.eliminar", function(){
			...
			function(isConfirm){
			  	if (isConfirm) {
			    	...
			    	scrollToTop();
			    	$.ajax({
						url:carpeta+'admin/paises/'+data.id,
						headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
						type: 'DELETE',
						dataType: 'JSON',
						data:{
