	...
	*/
	function listar(cuadro){
		...
		var table=$("#tabla").DataTable({
			...
			"ajax":{
				"method":"POST",
				"url":carpeta+"admin/gradosinstrucciones/listing",
			...
			"buttons":[
				{
					...
					"titleAttr": "Agregar Grado de Instrucción",
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_agregar(){
		$("#grado_instruccion_registrar").val("").focus();
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function registrar(){
		$("#form_registrar").submit(function(e){
			...
			$.ajax({
				url:carpeta+'admin/gradosinstrucciones',
				...
				error: function (repuesta) {
			        ...
			        if(typeof errores=='undefined'){
				    	...
				    	for(var i=0; i<errores.grado_instruccion.length; i++){
				        	html+='<li>'+errores.grado_instruccion[i]+'</li>';
				        }
				    }
			        html+='</div>';
					$("#alert").html(html);
	...
	*/
	function editar(tbody, table){
		...
		$(tbody).on("click", "span.editar", function(){
			...
			document.getElementById("grado_instruccion_actualizar").value=data.grado_instruccion;
...
/* ------------------------------------------------------------------------------- */
	...
	*/
	function limpiar_datos_editar(){
		$("#grado_instruccion_actualizar").val("").focus();
		$("#id_actualizar").val("");
	}
/* ------------------------------------------------------------------------------- */
	...
	*/
	function actualizar(){
		$("#form_actualizar").submit(function(e){
			...
			$('input[type="submit"]').attr('disabled','disabled');
			$.ajax({
				url:carpeta+'admin/gradosinstrucciones/'+document.getElementById('id_actualizar').value,
				headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
				type:'PUT',
				dataType:'JSON',
				data:form,
		...
		$(tbody).on("click", "span.eliminar", function(){
			...
			function(isConfirm){
			  	if (isConfirm) {
			    	...
			    	scrollToTop();
			    	$.ajax({
						url:carpeta+'admin/gradosinstrucciones/'+data.id,
						headers:{'X-CSRF-TOKEN': document.getElementById('token').value},
						type: 'DELETE',
						dataType: 'JSON',
						data:{
